...
const CartSideBar: FC<cartSideBarProps> = ({ value, setCart }) => {
  ...
  return (
    <div
      className={`h-screen bg-black-primary/60 w-full flex justify-end fixed z-20 transition-opacity ease-in-out duration-500 top-0 right-0 ${
        value ? "opacity-100 visible" : "opacity-0 invisible"
      }`}
    >
      <div
        ref={cartBox}
        className={`h-full overflow-y-auto bg-[#f2f2f3] w-[480px] absolute transition-all duration-300 border border-ash ${
          ...
          <div className="flex flex-col items-start justify-between h-[92vh]">
            <div className="bg-white pb-2 w-full">
              ...
              {cartItem?.map((item: any, i: number) => (
                <CartProduct key={i} data={item} index={i} />
              ))}
            </div>
