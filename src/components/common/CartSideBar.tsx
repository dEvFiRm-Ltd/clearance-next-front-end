...
const CartSideBar: FC<cartSideBarProps> = ({ value, setCart }) => {
  const { cartItem, totalPrice }: any = useCart();
  ...
  return (
    ...
    >
      ...
      >
        ...
        {cartItem?.length > 0 &&<div className="flex flex-col items-start justify-between h-[92vh]">
          <div className="bg-white pb-2 w-full">
            ...
            {/* product  */}
            {
             cartItem?.map((item:any, i:number)=> <CartProduct key={i} data={item} index={i} />)
            }
          </div>
            ...
            <div className="flex items-end justify-between p-2">
              <div className="flex space-x-2 items-center">
                <input
                  ...
                  type="checkbox"
                />
                <p className="text-[16px] leading-[18px] text-left text-[#31353c]">
                  Selected (2)
                </p>
              </div>
              <div>
                <p className="text-[16px] leading-[18px] text-[#5d626a] text-right my-2">
                  Save: $15.3
                </p>
                <p className="text-">
                  Total: <strong className="text-red-700">${totalPrice.toFixed(2)}</strong>
                </p>
              </div>
            </div>
