...
const PaymentMethod = () => {
    ...
    useEffect(() => {
        const updateStateFromLocalStorage = () => {
            ...
            setGuest(
                !(
                    customer?.customerInfo?.customer_info?.is_email_verified ===
                        1 ||
                    customer?.customerInfo?.customer_info?.is_phone_verified ===
                        1
                )
            );
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            setGuest(
                JSON.parse(localStorage.getItem('TOKEN_LOCAL_STORAGE'))?.guest
            );
            setUserData(
